...
function disparos (){
    if (keyIsDown(32)) {
        proyectilesActivos.push(true)
         for (let index = 0; index < proyectilesActivos.length; index++) {
            proyectilesActivos[index]
...
function finProyectil() {
    for (let index = 0; index < proyectilesActivos.length; index++) {
        if (posYProyectil[index] == 0) 
            proyectilActivo[index] = false; 
    }
